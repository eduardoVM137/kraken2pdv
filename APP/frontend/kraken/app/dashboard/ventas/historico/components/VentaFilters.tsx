...
export default function VentaFilters({
  ...
  clienteFilter, onClienteChange,
  usuarioFilter, onUsuarioChange,
  minTotal, onMinTotalChange,
  ...
  const toggleState = (s: string) =>
    onStateChange(stateFilter.includes(s) ? stateFilter.filter((x) => x !== s) : [...stateFilter, s]);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div>
          ...
          <Input type="date" value={dateFrom} onChange={(e) => onDateFromChange(e.currentTarget.value)} />
        </div>
        <div>
          <label className="block text-sm mb-1">Hasta</label>
          <Input type="date" value={dateTo} onChange={(e) => onDateToChange(e.currentTarget.value)} />
        </div>
        <div className="flex items-center">
          <Button variant="link" onClick={() => setShowAdvanced((v) => !v)}>
      ...
      {showAdvanced && (
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" className="w-full text-left">
                  {stateFilter.length ? stateFilter.join(", ") : "Seleccionar estados"}
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent>
          ...
          <div>
            ...
            <Input value={clienteFilter} onChange={(e) => onClienteChange(e.currentTarget.value)} placeholder="ID o nombre" />
          </div>

          <div>
            <label className="block text-sm mb-1">Usuario</label>
            <Input value={usuarioFilter} onChange={(e) => onUsuarioChange(e.currentTarget.value)} placeholder="ID o nombre" />
          </div>

          <div>
            <label className="block text-sm mb-1">Total (min / max)</label>
            <div className="flex gap-2">
              <Input type="number" value={minTotal} onChange={(e) => onMinTotalChange(e.currentTarget.value)} placeholder="0.00" />
              <Input type="number" value={maxTotal} onChange={(e) => onMaxTotalChange(e.currentTarget.value)} placeholder="0.00" />
            </div>
          </div>
