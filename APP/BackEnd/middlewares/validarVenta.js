...
const validarVenta = (req, res, next) => {
  const schema = Joi.object({
    ...
    comprobante: Joi.string().max(50).optional().messages({
      'string.max': 'El comprobante no debe exceder 50 caracteres',
    }),
    ...
    comentarios: Joi.string().max(150).optional().messages({
      'string.max': 'Los comentarios no deben exceder 150 caracteres',
    }),
  });
